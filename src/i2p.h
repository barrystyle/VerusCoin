...
/**
 * Maximum message size for I2P network.
 */
static const int64_t MAX_MSG_SIZE = 65536;
...
private:
     ...
     * @throws std::runtime_error if an error occurs
     */
    Sock Hello() const EXCLUSIVE_LOCKS_REQUIRED(cs);
     ...
     * @throws std::runtime_error if an error occurs
     */
    void DestGenerate(const Sock& sock) EXCLUSIVE_LOCKS_REQUIRED(cs);
     ...
     * @throws std::runtime_error if an error occurs
     */
    void GenerateAndSavePrivateKey(const Sock& sock) EXCLUSIVE_LOCKS_REQUIRED(cs);
     ...
     */
    Binary MyDestination() const EXCLUSIVE_LOCKS_REQUIRED(cs);
     ...
     * @throws std::runtime_error if an error occurs
     */
    void CreateIfNotCreatedAlready() EXCLUSIVE_LOCKS_REQUIRED(cs);
     ...
     * @throws std::runtime_error if an error occurs
     */
    Sock StreamAccept() EXCLUSIVE_LOCKS_REQUIRED(cs);

    /**
     * Destroy the session, closing the internally used sockets.
     */
    void Disconnect() EXCLUSIVE_LOCKS_REQUIRED(cs);
     ...
     */
    Binary m_private_key GUARDED_BY(cs);
     ...
     */
    Sock m_control_sock GUARDED_BY(cs);

    /**
     * Our .b32.i2p address.
     * Derived from `m_private_key`.
     */
    CService m_my_addr GUARDED_BY(cs);

    /**
     * SAM session id.
     */
    std::string m_session_id GUARDED_BY(cs);
};
