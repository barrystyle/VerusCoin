                                               ...
                                               bool check_result_ok) const
{
    ...
    // signaled.
    static int64_t recv_timeout = 3 * 60;

    reply.full = sock.RecvUntilTerminator('\n', recv_timeout, MAX_MSG_SIZE);

    const std::string &sp = reply.full;
    auto replySplit = Split(sp, ' ');
    for (const auto& kv : replySplit) {
...
{
    SOCKET sock = CreateSocket(m_control_host);
    ...
    Sock refSock;
    refSock.m_socket = sock;

    SendRequestAndGetReply(refSock, "HELLO VERSION MIN=3.1 MAX=3.1");
...
void Session::CreateIfNotCreatedAlready()
{
    ...
    std::pair<bool, std::string> result = ReadBinaryFile(m_private_key_file);

    bool read_ok = result.first;
