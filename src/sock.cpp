...
bool Sock::Wait(int64_t timeout, Event requested, Event* occurred) const
{
#ifdef USE_POLL
    pollfd fd;
    fd.fd = m_socket;
...
{
    const auto deadline = GetTime() + timeout;
    size_t sent{0};
    ...
    for (;;) {
        ...
        // if nothing was sent.
        const auto wait_time = std::min(deadline - now, int64_t{MAX_WAIT_FOR_IO});
